...
public class ProcesoDos {
    ...
    static class TareaRunnable implements Runnable {
        ...
        @Override
        public void run() {
            while (tarea.tiempoRestante > 0) {
                System.out.println("Ejecutando: " + tarea.getNombre());
                int tiempoEjecucion = Math.min(quantum, tarea.tiempoRestante);
                try {
    ...
    public void agregarTareas(int numTareas) {
        ...
        for (int i = 0; i < numTareas; i++) {
            ...
            Tarea tarea = new Tarea(nombre, prioridad, tiempoEjecucion, 0); //Creo la tarea
